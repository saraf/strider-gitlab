...
function get(config, uri, done) {
  ...
  superagent
    ...
    .end(function (error, res) {
      res = res || {};
      debug('get received Response body type: ' + typeof res.body);
      inspect('get received Response body : ', res.body);
...
function addDeployKey(config, repo_id, title, key, callback) {
  ...
  // If the configuration has no GitLab API URL, bail.
  if (!config.api_url) {
    return callback(new Error('API URL missing from GitLab provider configuration!'));
  }
  ...
  superagent
    ...
    .end(function (err, res) {
      ...
      async.each(res.body, function (key, cb) {
        // Remove all deploy keys matching url
        if (key.title == title) {
